...
// Grainient, de React Bits. Degradado animado con grano de película.
// Los colores por defecto salen de la paleta de ALARYFES.
...
// Renderer y programa sobreviven a los re-renders: el segundo efecto solo
// actualiza uniforms, nunca reconstruye el contexto WebGL.
...
export function Grainient({
  ...
  // Efecto 1: crea el contexto WebGL una vez y lo pausa fuera de pantalla
  // o con la pestaña oculta.
  useEffect(() => {
    ...
    // Con movimiento reducido se queda un fotograma fijo: el fondo sigue
    // ahí, pero no se mueve.
    ...
    const tryStart = () => {
      if (!quieto && isVisible && isPageVisible && raf === 0)
        raf = requestAnimationFrame(loop);
    };
    const tryStop = () => {
      if (raf !== 0) {
  ...
  // Efecto 2: props a uniforms, sin desmontar nada.
  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;
    ...
    // Un fotograma con los valores reales, por si el bucle está parado.
